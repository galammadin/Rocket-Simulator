...
//			double previous_reward = StateAndReward.getRewardAngle(previous_angle, previous_vx, previous_vy); //PART II
			double previous_reward = StateAndReward.getRewardHover(previous_angle, previous_vx, previous_vy); // PART III
			action_counter = 0;
...
//					String tempState = new_state + i;
//					if (Qtable.get(tempState) != null) {
//						tempVal = Math.max(tempVal, Qtable.get(tempState));
//					}
//				}
				
				Qtable.put(prev_stateaction, Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction)) 
